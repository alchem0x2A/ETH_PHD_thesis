...
def plot_main():
    fig, ax_all = gridplots(2, 2,
                        ratio=1.25)
    # ax_img.text(x=0.85, y=0.95, s="B=Si$_{3}$N$_{4}$", ha="center")
    # ax_img.text(x=0.85, y=0.75, s="A=SiO$_{2}$", ha="center")
    # ax_img.text(x=0.78, y=0.85, s="m", ha="center")
    ...
    ax_in1.scatter(res[:, 0], res[:, 1], s=5 ** 2, c=res[:, 0],
    ...
    grid_labels(fig, ax_all, reserved_space=(0, -0.05),)
    ax_img = inset_axes(ax_all[0], width="110%", height="130%",
                        loc="center")
    ax_all[0].set_axis_off()
    add_img_ax(ax_img, img_path / "3D" / "AFM_scheme.png", index=1)
    # labels = ["a", "b", "c"]
