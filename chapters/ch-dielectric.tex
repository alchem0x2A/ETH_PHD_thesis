...
\AA{} into the vacuum ( \autoref{fig-1}{\textbf a} and \worktodo{Supplementary}
...
alike results (see \worktodo{Supplementary}  \autoref{SI-fig:GW-PBE-alpha}).
...
many-body screening and excitonic effects (details see \worktodo{Supplementary}
...
\worktodo{Supplementary} ~\autoref{SI-fig:rho-profile}.
%
%
...
% panels in \autoref{fig-1}{\textbf b} and \autoref{fig-1}{\textbf c} for
% $\varepsilon_{\mathrm{SL}}^{\parallel}$ and
...
respectively (details see \worktodo{Supplementary} Section \autoref{SI-ssec:diel-gw}).
%
% 
% 
% 
...
frequency-dependent domain can be found in \worktodo{Supplementary} Section
...
ellipsometry data
\cite{graphene-epsilon10,Duesberg_2014_opt_MoS2,Chiang13,}\worktodo{cite
  these 2}, reflectance / transmission spectra \cite{Li_2014_opt_ml_mos2,
...
\worktodo{Supplementary} ~\autoref{SI-fig:rescale-prb}). The fitted values of
the slab thickness, $\delta_{\mathrm{2D}}^{\parallel, \mathrm{fit}}$
and $\delta_{\mathrm{2D}}^{\perp, \mathrm{fit}}$ from in-plane and
out-of-plane data, respectively, are shown in \worktodo{Supplementary}
...
generally at the order of 10\textsuperscript{-4} (\worktodo{Supplementary}
...
\worktodo{Supplementary} Section \autoref{SI-sec:diel-theory-1} for details):
% \begin{subequations}
% \begin{eqnarray}
...
\worktodo{Supplementary} Section \autoref{SI-sec:diel-pol-2D-Eg}).  We then examine
...
that from PBE exchange-correlation functional (see \worktodo{Supplementary}
Section \autoref{SI-sec:diel-pol-2D-Eg} and \worktodo{Supplementary} 
...
in \worktodo{Supplementary} Section~\autoref{SI-sec:diel-gpaw}.


We further examine the validity of ~\autoref{eq:diele-2D-Moss-perp}, i.e.,
...
material. \worktodo{Supplementary} Section
...
(\worktodo{Supplementary} Section \autoref{SI-sec:diel-gpaw-3}).
...
\cite{Pedersen_2016_shark_effect_TMDC,Klein_2016_stark_eff,Roch_2018_stark_eff}.\worktodo{cite Eda group}
...
anisotropic.  \autoref{fig:aniso} shows the phase diagram of $\eta$
...
transparency phenomena \worktodo{cite previous sections }\cite{Li_2014_screen} and the large exciton
binding energies
...
\autoref{eq:diele-anisotropy} we can see $\eta$ is roughly proportional to
...
$E_{\mathrm{g}}$ (0.1--7 eV) ( \autoref{fig-3}b and
\autoref{fig-3}c). Further analysis shows that the dielectric anisotropy
...
\worktodo{Supplementary} Section \autoref{SI-sec:diel-aniso}), which is the basis for the
...
dimensions (details see \worktodo{Supplementary} Section \autoref{SI-sec:diel-aniso}) on
the phase diagram in  \autoref{fig:diele-aniso}. Bulk covalent (3D,
...
(vdWH) \cite{Novoselov_2016_vdW}), provides more flexibility in
...
\caption{\label{fig-1} \worktodo{more} \textbf{2D polarizability and the breakdown of
...
\begin{figure}[!htbp]
  ...
  \caption{\label{fig-emt} \worktodo{more} \textbf{Failure of the effective 2D
