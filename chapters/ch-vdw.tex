...
such that $\Phi_{\mathrm{AmB}} \approx \Phi_{\mathrm{vdW}}$.  If we further
neglect the retardation effect~\autocite{Dryden_2015_gecko} (due to
...
uniform within each material and is a tensor
...
thickness of the 2D material, such that a uniform description of
...
%
To characterize the decaying of $\varepsilon_{\mathrm{m}}^{\parallel}$ $\varepsilon_{\mathrm{m}}^{\perp}$, we introduce the in- and
...
%
Conversely, an increase of \(\eta^{\mathrm{vdW}}\) can also be achieved
by lowering both \(E_{\mathrm{g}}^{\mathrm{A}}\) and
\(E_{\mathrm{g}}^{\mathrm{B}}\) simultaneously.
%
Clearly, the results in \autoref{fig:vdw-database} shows a systematic
...
\begin{figure}[!htbp]
  ...
  \caption{\label{fig:vdw-g-dist}%
    ...
    and \textbf{iii}: cubic BN). The color of curves indicates the
    magnitude of $d$.  }
\end{figure}
...
dielectric cascade, such that $\hat{\varepsilon}_{\mathrm{A}} < \hat{\varepsilon}_{\mathrm{m}} <
...
As a model study, we experimentally compare the molecular epitaxy of a
conjugated organic molecule (N,N’-bis(2-phenyl\-ethyl)-3,4,9,10-
peryl-ene\-tetra\-carboxylic, BPE) on top of two different substrates:
SiO\textsubscript{2}-supported graphene (Gr/SiO\textsubscript{2}) and
gold-supported graphene (Gr/Au).  The structure of BPE features two
phenyl\-ethyl groups at the side of the PTCDI basal plane which have
rotation freedom.
%
...
thickness of the film, instead of only probing the first few layers.
%
...
shrinks with increasing $d$, and can  be smaller than
...
\autoref{fig:vdw-repul-suspen}\lc{b}, when $\Phi_{\mathrm{AmB}}$ becomes repulsive for gold deposited on both suspended graphene and hBN
sheets.
%
...
data\autocite{Palik_1998_handbook} or using first principle simulations.
%
47 bulk materials have their dielectric properties extracted from
Ref. \cite{Palik_1998_handbook}.
%
...
measurements. E.J.G.S. helped with the dielectric calculations and
participated in the discussion of Lifshitz-vdW model.
