...
% If we assume $\varepsilon_{i}^{xx} = \varepsilon_{i}^{yy}$, the dependency of $g_{\mathrm{m}}^{2}$ on $\vartheta$ is removed.
...
Based on the SLOM, \autoref{fig:vdw-compare-slom}b(i)-b(iii) shows the 2D
...
overwhelms the vdW
interactions~\citetitle{Israelachvili_2011_book}. We also comment that
...
%
Next we examine the molecular packing on these surfaces using GIXD. As
shown in \autoref{fig:vdw-bpe}c and \autoref{fig:vdw-bpe}d, in th 20
nm-thick BPE films deposited onto both Gr/SiO\textsubscript{2} and
Gr/Au surfaces, the major peaks in the GIXD spectra appears around 19
nm$^{-1}$ near the $q_{z}$ axis, corresponding to the diffraction from
the (1$\overline{2}$4) and (1$\overline{2}1$) 
lattice planes~\cite{Hadicke_1986_BPE,Mizuguchi_1998_BPE}.
%
...
%
\worktodo{appropriate to use $\Phi_{\mathrm{AmB}}$ still?}
