...
wettability of graphene has been observed in the contact-angle
experiments
\cite{Hong_2016_mechanism,goniszewski_correlation_2016,Ashraf_2016_doping},
and the molecular dynamics (MD) simulations
\cite{Ostrowski_2014_tunable,Ren_2015_interfacial,Taherian_2015_asym_EW,Daub_2007_nanoscale_EW}. In
...
\cite{rafiee_2012_transparency,shih_2013_wetting_natmat}, does not affect our
...
following 2D materials are considered: TMDC monolayers
(MX\(_{\text{2}}\), M = Mo, W and X = S, Se, Te), silicene, germanene,
phosphorene (monolayer black phosphorus), and graphene. The doping
density in a 2D material is calculated by integrating the DOS from its
intrinsic Fermi level, i.e.,
\(\sigma_{\mathrm{2D}} = \int_{E_{\mathrm{F,0}}}^{E_{\mathrm{F}}}
g(E')e \mathrm{d}E'\) \cite{John_2004_QC}.
...
\cite{shih_2013_wetting_natmat}, at \(\sigma_{\mathrm{2D}}\) =
...
than that in the 2D semimetals \cite{Davies_1997_book},
thereby resulting in high DOS, as well as a high \(C_{\mathrm{2D}}\)
(see eq \autoref{eqn:wet-delta-gamma-sigma-free-2D}). This concept also
explains why the surface energy decrease for silicene and germanene
are lower than that for graphene \cite{Yan_2013_e-hv-couple}.  To our knowledge,
...
liquid molecules \cite{Ostrowski_2014_tunable} and (ii) the formation of
...
solutions, e.g., pure water with self-ionized
H\(_{\text{3}}\)O\(^{\text{+}}\) and OH\(^{\text{-}}\) ions. Here we
consider the graphene-water interface as a model system. All MD
simulations were carried out using the GROMACS 4.5 software package
...
\cite{Tummala_2008_counterion_SDS}. The doping effect is included by
assigning an equal amount of charge
...
0.012 \(e/\mathrm{atom}\). Water molecules were modeled using the
SPC/E model \cite{Berendsen_1987_pair} with bond lengths and angles of
water molecules constrained using the SETTLE algorithm
...
\autoref{fig:wet-MD-res}(a) and \autoref{fig:wet-MD-res}(b)). Note that the current
...
\autoref{eqn:wet-def-Young-Delta-theta}, the change of interfacial energy
...
SPC/E model \cite{Vega_2007_tension}. Note that the value of
\(\gamma_{\mathrm{L}}\) only affects the results of
\(\Delta \cos \theta\) while not the interaction potential \(\Phi\), as revealed by eq
\autoref{eqn:wet-Delta-Phi-Delta-cos}. Moreover, even though the surface
...
\autoref{fig:wet-MD-res}(c) shows the calculated \(\Delta \Phi\), and
...
\mathrm{\AA}\)) \cite{Mark_2001_MD_water}. Increasing the doping density of the
...
equilibrium distance, as seen in S2(a), the distance between
the first water layer and graphene (calculated from maximal
\(\rho_{\mathrm{L}}\) from \autoref{fig:wet-MD-res}(d)) slightly decreases
...
\(\sigma_{\mathrm{2D}}\) in \autoref{fig:wet-MD-res}(c) shows an apparent
...
shown in \autoref{fig:wet-MD-res}(d) and \autoref{fig:wet-MD-res}(e),
...
dipole orientation \(\cos \mu\) (S1) profiles, both
...
S3) also matches those observed in \(\rho_{\mathrm{L}}\),
...
adsorption theory \cite{Bard_1980_electrochem_book}, one can obtain:
\begin{equation}
\label{eq:2}
...
capacitance. Combining eq \autoref{eqn:wet-Delta-gamma-2DL-EDL-full} with eqs
...
\end{equation}
In other words, eqs \autoref{eqn:wet-delta-gamma-sigma-free-2D} and
...
\autoref{fig:wet-res-EDL}(a).  The EDL consists of a Helmholtz layer with the
...
\cite{Bard_1980_electrochem_book}. The \(C_{\mathrm{EDL}}\) of the
...
\autoref{eqn:wet-neutrality}) still holds, such that \((\Delta \cos
\theta)^{\mathrm{EDL}}\) remains the same (see eq
\autoref{eqn:wet-Delta-cos-EDL-final}). In other words, despite the recent
...
\cite{Smith_2016_screening,Lee_2017_scaling}, which
...
\cite{Mugele_2005_EW_rev}. Moreover, within the range of
...
n- and p-doped regimes, respectively (see S5), and use the
...
Figures \autoref{fig:wet-f-nc-exp}(a) and \autoref{fig:wet-f-nc-exp}(b) illustrate how
...
C_{\mathrm{d}}\) in eq \autoref{eqn:wet-def-Delta-cos-mixture}, respectively. The
...
\(\sigma_{\mathrm{2D}}\) using eq \autoref{eqn:wet-def-Delta-cos-mixture}. Figure
...
\autoref{fig:wet-dcos-all-2D}(a). Specifically,  eqs \autoref{eqn:wet-psi-GC} and
