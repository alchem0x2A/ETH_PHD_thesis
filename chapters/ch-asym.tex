...
\subsection{Classical electrostatic approach}
...
assumption. \worktodo{Figure} 6(a) schematically shows the band diagram of the
...
function of layer i, and (iv) the work function difference between two
...
of 1L G/ 1L MoS\textsubscript{2}, we plot in \worktodo{Figure} 6(b) the work functions φ\_i of
...
with the findings in \worktodo{Figure} 3(a). We ascribe such asymmetry to the
...
transfer is mainly determined by the position of the Fermi level with
respect to the CB or VB of MoS\textsubscript{2}. Following the same procedure, we
calculated the dipole moment (\worktodo{Figure} 6(c)), and the charge density of
the graphene layers (\worktodo{Figure} 6(d)) for a different number of layers at
...
calculations of dipole moment (\worktodo{Figure} 5(a)) and charge transfer
(\worktodo{Figure} 5(b)): the charge redistribution is more pronounced with
...
fields of -2 V/nm (\worktodo{Figure} 6(e)) and +2 V/nm (\worktodo{Figure} 6(f)). Under -2
...
calculations showed in \worktodo{Figure} 5(d)-5(f). Charge accumulation occurs
