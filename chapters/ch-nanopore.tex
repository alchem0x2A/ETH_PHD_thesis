...
\end{equation}%
\worktodo{figure ?}  which gives $\delta$ in the range of $1\sim{}5$
...
% The theoretical diffusive fluxes through pores in PCTE are obtained
% using cylindrical pore model with uniform diameter and a low
% tortuosity $\tau \sim{} 1.2$ \cite{O_Hern_2012_defect}:
% \begin{equation}
%   \label{eq:np-j-pcte}
%   \symbf{J}_{\mathrm{PCTE}} = \frac{\pi r_{\mathrm{PCTE}}^{2} N_{\mathrm{p}} D_{\mathrm{\pm}} \Delta c}{L \tau}
% \end{equation}
% {where $r_{\mathrm{PCTE}}=$ \unit[200]{nm} is the average PCTE pore radius, $N_{\mathrm{p}} =$ \unit[$1.5\cdot10^{12}$]{m$^{-2}$}) is
%   the numbers density of pores per area, $\mathcal{D}_{\mathrm{\pm}}$ is the average salt diffusive, $\Delta c$ is the bulk concentration difference cross the membrane, \ie
%   $\Delta c \sim c_{0}$ and $L =$ \unit[24]{μm} is the thickness of the PCTE membrane.
%   %
...
% Using a simple circuit model, we estimate the
% ratio $\delta$ between $R_{\mathrm{G}}^{0}$ and $R_{\mathrm{PCTE}}$,
% i.e., $\delta = R_{\mathrm{PCTE}}/R_{\mathrm{G}}^{0}$, to be
% $\sim$2.2, which is in good agreement with the value derived from the
% experimental data in \autoref{fig:np-2}b (details see Supplementary
% Section \autoref{SI-sec:np-R-model}).
...
Stern layer \worktodo{ref to wetting part}, and $\varepsilon_{0}$ is
the vacuum permittivity.
%
Note although the carrier concentration may not essentially follow the
Poisson-Boltzmann distribution, due to the huge concentration different between the two reservoirs \worktodo{cite one}.
%
...
residue is minimal after the thermal annealing process\worktodo{cite 1}.
%
The theory
...
\autoref{eq:np-electro-neutral}-\autoref{eq:np-delta-phiG} that were
...
\autoref{fig:np-4}b, respectively. \worktodo{split to 2 figures}
%
...
%
\worktodo{attach the figures from SI}
...
balance between the diffusion and drift potentials \worktodo{move SI figure S8 here}.
%
The
...
values. Note a negative $\symbf{J}_{z}$ means flux from HCR ($z>0$)to LCR ($z<0$) in the simulation box. \worktodo{where to put the simulation box?}
%
...
\worktodo{move SI figure S10 and S11 here?}.
%
Intriguingly,
...
nm\cite{Jain_2015_subcontin_ion}.  Advanced
  ...
  We notice that the calculations in Figs. \autoref{fig:np-4} and
...
the salt rejection, \autoref{eq:np-rejection-comb} is combined with \worktodo{which figure}, where
the pore distribution is taken from the SEM image of real samples (\worktodo{which figure inser}).
%
...
$r_{\mathrm{G}}$ and $V_{\mathrm{G}}$. As seen in \worktodo{to 2D plot}, in large $V_{\mathrm{G}}$ and
...
\autoref{fig:np-5}b).  \worktodo{z for coordinate and also valency?}
...
%
% It is
%   worth noting that the observed linearity can only describe the
%   $\lambda_{\mathrm{D}}$ range considered here. When
%   $\lambda_{\mathrm{D}}$ increases beyond 30 nm, this linear relation
%   would yield a $\xi_{\mathrm{max}}$ value larger than 1, which
%   appears to be unphysical.  More follow-up experimental and
%   theoretical efforts are required to further investigate the
%   small/large $\lambda_{\mathrm{D}}$ regimes. Also note that the salt
%   diffusivity (see (4)) and the specific surface adsorption of ions
%   may also influence the experimentally observed $\xi_{\mathrm{max}}$
...
\begin{figure}[H]
  \centering
  \import{\imgdir}{rejection_contoursvg.pdf_tex}
...
\begin{figure}[!htbp]
  \import{\imgdir}{pore_dist.pdf_tex}
\end{figure}
