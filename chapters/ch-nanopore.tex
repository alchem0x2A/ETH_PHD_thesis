...
% \textcolor{blue}{In this report, we investigate electrolyte permeation
  % through cm$^2$ scale graphene membranes having pores that are
  % $<$\unit[50]{nm}. We find, that an applied voltage bias can
  % effectively modulate the ion transport through these membranes,
  % despite having pores significantly larger than the hydration radii
  % of the ions.  We simulate the surface potential, taking into account
  % the elementary electronic properties of graphene, allowing to model
  % and confirm that a high degree of salt rejection could be achieved
  % by both pore sizing and electrostatic gating. Our analysis reveals a
  % large impact of atomistic effects, such as quantum capacitance, on
  % the ionic transport through gated porous 2D materials, providing new
...
% \textcolor{blue}{Furthermore, the specific adsorption of ions
  % \cite{bard_electrochemical_1980} on porous graphene surface with
 % on highly-charged sites \cite{Rollings_2016} is ignored in the current
  % model due to (i) low intrinsic charge density of porous graphene
  % fabricated using the BCP-patterning method and (ii) the surface charge
  % on graphene being effectively dominated by gating (Supplementary
  % Section \ref{SI-sec:charge-dens}). A full kinetic description of
  % such interactions should, however, be included in the model in case 
  % highly-adsorbing species such as fluoride or organic ions are
  % involved \cite{He_2012}.}
